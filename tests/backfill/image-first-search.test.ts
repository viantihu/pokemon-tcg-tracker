 ...
 * because she recognises artwork before she reads a name. #212 built `CardResultsGrid` with `CardLookup`'s
 * exact `{ search, onPick, placeholder }` contract so a site migrates by changing one tag; this pins that
...
describe("UIL-071 · every Backfill type-ahead is the image-first grid", () => {
  it("the text-list component is neither imported nor rendered anywhere on the screen", () => {
  ...
  it("each site passes exactly CardLookup's contract: search, onPick, placeholder", () => {
