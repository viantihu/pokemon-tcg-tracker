-- 0024_file_total_check — the record and the queue together must add up to the Dex file (UIL-100 hardening).
--
-- Build contract: docs/dev-spec.md §4 (migrations are FORWARD-ONLY and ordered; RLS on every table).
-- Additive — 0001–0023 are FROZEN; never edit them (docs/devops-strategy.md §6, dev-spec §4).
--
-- THE GAP. 0022's `assert_presence_counts` compares each card's copies with the Dex record, so a write that
-- adds the same amount to BOTH is invisible to it. The live instance (found by Dev 2, closed by #330's status
-- check) was a second press of Match on a row already matched: the record and the copies each grew by the
-- row's quantity. Nothing at the database stopped the next write of that shape.
--
-- THE RULE. Every Dex quantity lives in exactly one place — the record, or the queue (WAITING / DISMISSED) —
-- so `dex_import.file_total = sum(dex_presence.quantity) + sum(queue quantity)` after every write except an
-- import, which rewrites the record and the queue together. A write that adds to the record without moving
-- the same quantity out of the queue is refused, whole. The Sync page already computes the same sum at rest
-- (lib/sync/count-check.ts `fileAddsUp`), in the same words, so she never meets a refusal it did not warn of.
--
-- WHO EMITS IT, LAST: a manual match and a stand-in match (via matchOps), and a Retry — the three writers
-- that add to the record. Not an import (it rewrites both sides), not "add it back" (it moves neither), and
-- never placement, moves or collection edits.
--
-- Expected row effect: NONE. No table changes, no data written; one new function branch.
--
-- COMPOSED FUNCTION, READ WITH ITS NEIGHBOURS. 0023 only altered `copy`, so 0022's body is the one that runs
-- before this file. This file replaces `apply_write_ops` with 0022's body VERBATIM plus the one branch marked
-- "NEW in 0024", and nothing else; tests/sync/file-total-check.test.ts asserts exactly that.
--
-- SECURITY properties are preserved EXACTLY as 0006–0022 declared them: `security invoker`,
-- `set search_path = public, pg_temp`, no `auth.uid()` in the body, NO dynamic SQL, unknown op still raises.
