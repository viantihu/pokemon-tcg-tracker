-- 0008_collection_removal_ops — extend the single-transaction write applier so a card can be
-- REMOVED from a collection atomically (UIL-014).
--
-- Build contract: docs/dev-spec.md §4 (migrations are FORWARD-ONLY and ordered; RLS on every table).
-- Additive — 0001–0007 are FROZEN (applied to live prod + testing); never edit them
-- (docs/devops-strategy.md §6, dev-spec §4).
--
-- WHY. `copy` has no `collection_id` (0002_domain.sql): a card is "in" a collection because a shelved
-- copy sits in one of the collection's binders AND the card is on `collection.target_catalog_card_ids`.
-- There is no field to clear, so removal is a MOVE plus a list edit — two writes that must not be able
-- to half-apply. Half-applied either way is corrupt: rewrite the placement and lose the list edit and
-- the card reappears as un-owned; drop the list entry and lose the placement rewrite and the copy is
-- still shelved in the collection's binder while being invisible in every collection and wishlist view
-- (both keyed off `target_catalog_card_ids`) — an untracked physical card in a real pocket.
--
-- 0006/0007 routed the haul, sync and backfill commits through `apply_write_ops` and dropped their
-- compensating rollbacks, but neither op set could express this path: there was a
-- `union_collection_targets` and no inverse, and no way to patch `evolution_line.status`.
--
-- `apply_write_ops` is a single function, so extending it means REPLACING it: the body below is
-- 0007's body verbatim plus one new local (`ids`) and exactly two new op branches —
--
--   1. `subtract_collection_targets` — the inverse of 0007's union. One statement whose new value is
--      derived from the column itself, so two interleaved edits compose instead of clobbering.
--
--   2. `update_line` — patch a line's `status`, so a removal that vacates the last filled slot can
--      demote a `complete` line back to `open` INSIDE the same transaction (removal symmetry,
--      sync-architecture §1.6). Partial-patch semantics match `update_copy` / `update_slot`: a key
--      present is written, a key absent leaves the column alone.
--
-- Nothing else changes: no branch is removed or altered, and no pure logic moves into SQL — the TS
-- layer still computes the whole ordered write set (lib/coll/remove.ts).
--
-- SECURITY properties are preserved EXACTLY as 0006/0007 declared them: `security invoker` (0002's
-- `owner_all` RLS governs every write, and a collection that is not the caller's matches no row),
-- `set search_path = public, pg_temp`, `owner_id` never read from the payload, NO dynamic SQL (every
-- table and column name is a literal chosen by a fixed `case`), and an unknown op raises so a typo
-- fails the whole transaction rather than silently writing less.
--
-- `create or replace function` preserves the object's existing privileges, so 0006/0007's grants
