-- 0022_dex_record — the app keeps its own record of what the Dex file said, and every sync write is
-- checked against it before it commits (UIL-100).
--
-- Build contract: docs/dev-spec.md §4 (migrations are FORWARD-ONLY and ordered; RLS on every table).
-- Additive — 0001–0021 are FROZEN; never edit them (docs/devops-strategy.md §6, dev-spec §4).
--
-- KARVI'S REQUIREMENT, her words: "There needs to be some kind of validation in the sync that makes sure
-- that the total number of cards in the collection equal the dex import file. This will ensure that cards
-- are not double counted during the matching process or manual add process."
--
-- WHY NOTHING COULD CHECK THIS BEFORE. The only record of what Dex said lived in the CSV, in the request of
-- the import that read it. `presence_group.desired_count` looks like that record and is not: 0006's resync
-- (restated in every re-issue since, last in 0021) overwrites it with the live copy count after every write,
-- so it can only ever agree with the copies. And a manual match or a Retry happens later, with no file in
-- hand. So a double could be committed, and nothing afterwards could tell.
--
-- WHAT THIS ADDS.
--   * `dex_presence`: one row per (card, Dex variant) with Dex's RAW quantity, before removals. A full import
--     replaces it; a Retry promotion and a manual match add the row they resolve; Undo restores the record
--     the undone import replaced (from its undo snapshot).
--   * `dex_import`: one row per owner, the file-level total and row count of her last import, so the Sync
--     page can show the whole sum: file = in the collection + waiting to match + dismissed + removed.
--   * Five `apply_write_ops` branches. Three maintain the record; `shrink_removed_presence` takes back part of
--     a removal memory (Undo of a card she removed, and UIL-099 E2's "add it back"); `assert_presence_counts` is
--     THE CHECK: emitted last by every sync writer, it raises unless every key it names holds
--     max(0, dex - removed) copies, so a count that disagrees with Dex is never committed. That is the
--     same rule `applyRemovedMemory` applies for an import, so a removal never reads as a mismatch; the
--     TypeScript check calls that function and tests/sync/count-check.test.ts pins the two against each
--     other.
--   * Copies with no presence group have no key and are not checked here; the Sync page lists them
--     separately, and 0023 (UIL-098 part 4) makes them impossible.
--
-- Expected row effect on Testing: NONE. Two new empty tables, and branches only reachable from a sync
-- written after this deploys. With no `dex_import` header the check passes, so nothing that exists today
-- can be refused by it; her first import writes the header and turns it on.
--
-- COMPOSED FUNCTION, READ WITH ITS NEIGHBOURS. 0013, 0014, 0015, 0017, 0020 and 0021 each replaced
-- `apply_write_ops`. This file replaces it again with 0021's body VERBATIM plus the five branches marked
-- "NEW in 0022", and nothing else; tests/sync/dex-record-migration.test.ts asserts exactly that.
--
-- SECURITY properties are preserved EXACTLY as 0006–0021 declared them: `security invoker` (the new tables
-- carry 0002's `owner_all` policy shape), `set search_path = public, pg_temp`, `owner_id` never read from the
-- payload (it defaults to `auth.uid()`), NO dynamic SQL, and an unknown op still raises.

create table dex_presence (
  ...
  -- Dex's quantity for this exact (card, Dex variant) — raw, before removals.
...
comment on table dex_presence is
  'What her last Dex import said she owns, per (card, Dex variant), before removals (UIL-100). Replaced by a full import; added to by a Retry promotion or a manual match; restored by Undo. Keyed exactly like presence_group and removed_presence, so the check is max(0, quantity - removed) = copies with no second derivation.';

create table dex_import (
  owner_id    uuid primary key default auth.uid(),
  -- Sum of Quantity over the owned (Type = collection) rows of the file.
  file_total  integer not null check (file_total >= 0),
  row_count   integer not null check (row_count >= 0),
  imported_at timestamptz not null default now()
);

comment on table dex_import is
  'Her last Dex import, file level (UIL-100): total cards and rows. The Sync page checks file_total = sum(dex_presence.quantity) + waiting + dismissed. Absent until her first import after 0022; while absent, assert_presence_counts passes.';

alter table dex_presence enable row level security;
create policy owner_all on dex_presence
  for all to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());

alter table dex_import enable row level security;
create policy owner_all on dex_import
  for all to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
...
begin
  ...
  loop
    case op ->> 'op'
      ...
      -- statement below sees and touches only the caller's rows. No branch here calls `auth.uid()` itself;
      -- none of 0006-0021 ever did in a function body, and a first would be proven only in production. The
      -- two branches that DELETE a whole record refuse to run as anything but the signed-in owner, because
      -- the service role bypasses RLS and an unscoped delete would clear every owner's record.
